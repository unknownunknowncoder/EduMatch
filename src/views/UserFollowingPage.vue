  ...
  <div class="min-h-screen bg-[#F3F4F6] font-sans text-slate-800 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 mb-6">
        <h1 class="text-2xl font-bold text-slate-900 mb-2">{{ userInfo.nickname || userInfo.username }}的关注</h1>
      ...
      <div v-else-if="followingList.length === 0" class="bg-white rounded-xl border border-dashed border-slate-300 p-12 text-center">
        ...
        <h3 class="text-lg font-medium text-slate-900 mb-2">TA还没有关注任何人</h3>
        <p class="text-slate-500">这个用户还没有开始关注其他用户</p>
      </div>
...
const loadFollowingList = async () => {
  try {
    ...
    // 获取关注列表
    const data = await supabaseService.getFollowingList(targetUserId.value, {
