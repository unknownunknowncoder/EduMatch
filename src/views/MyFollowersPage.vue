  ...
  <div class="min-h-screen bg-[#F3F4F6] font-sans text-slate-800 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 mb-6">
        <h1 class="text-2xl font-bold text-slate-900 mb-2">我的粉丝</h1>
      ...
      <div v-else-if="followersList.length === 0" class="bg-white rounded-xl border border-dashed border-slate-300 p-12 text-center">
        ...
        <h3 class="text-lg font-medium text-slate-900 mb-2">还没有粉丝</h3>
        <p class="text-slate-500 mb-6">继续发布优质内容，吸引更多粉丝吧！</p>
        <button 
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-sm text-slate-500">
                  于 {{ formatDate(item.created_at) }} 开始关注你
                </p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              <button 
                v-if="!isFollowingMap[item.user.id]"
...
const loadFollowersList = async () => {
  try {
    ...
    // 获取粉丝列表
    const data = await supabaseService.getFollowersList(currentUser.id, {
    ...
    for (const follower of data) {
      ...
      followStatusMap[follower.user.id] = await supabaseService.isFollowing(followerId, followingId)
    }
