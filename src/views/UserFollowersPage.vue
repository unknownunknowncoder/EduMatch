  ...
  <div class="min-h-screen bg-[#f2f0e9] font-sans selection:bg-[#1a3c34] selection:text-[#e8e4d9] pb-24">
    ...
    <div class="h-64 relative w-full border-b-4 border-[#1a3c34]">
      ...
      <div class="absolute bottom-0 left-0 w-full p-8 z-10">
        <div class="max-w-6xl mx-auto">
          <h2 class="text-[#d4c5a3] text-xs font-bold uppercase tracking-[0.3em] mb-2">Followers Records</h2>
          <h1 class="text-3xl font-serif font-bold text-white tracking-wide">{{ userInfo.nickname || userInfo.username }}'s Followers</h1>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-6xl mx-auto px-6 mt-8 relative z-10 space-y-8 mb-24">
      ...
      <div v-if="loading" class="bg-white p-12 shadow-xl border-t-8 border-[#1a3c34] text-center">
        ...
        <p class="mt-4 text-[#1a3c34]/60 font-serif italic">Loading followers records...</p>
      </div>

      <!-- 隐私保护状态 -->
      <div v-else-if="privacySettings.hide_followers" class="bg-white p-12 shadow-xl border-t-8 border-[#b03e3e] text-center">
        ...
        <p class="text-[#1a3c34]/60 mb-6">This scholar's followers list is private.</p>
         ...
         <div class="bg-white shadow-xl border border-[#1a3c34]/10">
            <div class="p-6 border-b border-[#1a3c34]/10">
               <h3 class="text-lg font-serif font-bold text-[#1a3c34]">
                  <span class="text-[#d4c5a3]">{{ followersList.length }}</span> Followers
               </h3>
            </div>
            
            <div v-if="followersList.length === 0" class="text-center py-16">
               <UserX class="w-12 h-12 text-[#1a3c34]/20 mx-auto mb-4" />
               <p class="text-[#1a3c34]/60 font-serif">No followers yet.</p>
            </div>
...
const fetchUserInfo = async () => {
  try {
    ...
    userInfo.value = { ...user }
    await Promise.all([fetchFollowersList(), fetchPrivacySettings()])
  } catch (error) {
...
const fetchFollowersList = async () => {
  try {
    ...
    const { data } = await client
      ...
      `)
      .eq('following_id', userInfo.value.id)
    
    if (data) {
      followersList.value = data.map(item => item.users).filter(Boolean)
    }
  } catch (error) {
