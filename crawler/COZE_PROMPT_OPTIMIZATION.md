...
你遇到的核心问题：
1. **平台偏向性严重** - 智能体过度依赖B站API，忽视MOOC知识库
2. **知识库利用率低** - 有丰富MOOC数据却优先使用外部API
3. **推荐不均衡** - 违背"多平台全面匹配"的设计目标
4. **一致性差** - 有时查知识库，有时用API，用户体验不稳定
...
```
测试用例1："Python爬虫学习"
期望结果：包含北理工/哈工大MOOC + B站爬虫教程

测试用例2："前端开发入门"  
期望结果：包含知名大学MOOC + B站前端教程

测试用例3："数据分析进阶"
期望结果：包含浙大/清华MOOC + B站数据实战项目
...
1. **立即更新系统提示词** - 使用上面的严格版本
2. **调整知识库权重设置** - MOOC权重设为0.6，B站为0.4  
3. **禁用或限制外部API** - 确保优先使用知识库
4. **测试验证效果** - 用多个测试用例验证
5. **持续监控优化** - 根据实际效果微调

这样修改后，你的智能体将严格遵守多平台均衡推荐的原则，充分挖掘MOOC和B站两个知识库的价值！