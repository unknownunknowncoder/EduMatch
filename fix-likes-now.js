...
async function fixLikesNow() {
  try {
    console.log('🔧 修复点赞功能...\n');

    // 1. 检查当前点赞表状态
    console.log('1. 检查点赞表状态...');
...
async function createLikesTable() {
  console.log('  创建 post_likes 表...');
  
  // 由于无法直接执行DDL，我们尝试通过插入数据来测试
  // 如果表不存在，插入会失败，但我们可以知道表结构需要修复
  ...
  if (error && error.code === '42P01') { // 表不存在
    console.log('  需要创建 post_likes 表，请通过Supabase控制台执行SQL：');
    console.log(`
...
async function checkAndFixRLS() {
  ...
  if (error && error.code === '42501') { // RLS策略拒绝
    console.log('  RLS策略限制插入，需要调整策略');
    console.log('  请通过Supabase控制台执行以下SQL来修复RLS：');
    console.log(`
      -- 删除现有策略
