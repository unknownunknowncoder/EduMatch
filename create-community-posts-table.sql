...
-- 添加注释
COMMENT ON COLUMN community_posts.tags IS '帖子标签数组，用于分类和搜索';
COMMENT ON COLUMN community_posts.comments_count IS '评论数量，通过触发器自动更新';
COMMENT ON COLUMN community_posts.author IS '作者昵称，默认为匿名用户';
COMMENT ON FUNCTION search_posts_by_tags IS '根据标签数组搜索帖子';
COMMENT ON FUNCTION get_popular_tags IS '获取热门标签统计';
COMMENT ON FUNCTION search_community_posts IS '全文搜索社区帖子';
